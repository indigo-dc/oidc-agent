...
- [liboidcagent-go](#liboidcagent-go): Library for the Go programming language,
...
know which account configuration should be used, but you know for which provider
you need an access token you can also [request an access token for a
provider](#requesting-an-access-token-for-a-provider2).
...
information use [```GetTokenResponse```](#gettokenresponse2).
