...
oidc_error_t revokeToken(struct oidc_account* account) {
  ...
  char* data = generatePostData(
      ...
      "token", account_getRefreshToken(*account),
      NULL
      );
  if(data == NULL) {
    return oidc_errno;
  ...
  char* res = sendPostDataWithBasicAuth(account_getRevocationEndpoint(*account), data, account_getCertPath(*account), account_getClientId(*account), account_getClientSecret(*account));
