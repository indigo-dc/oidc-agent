...
EXCHANGE_OBJECTS := $(EXCHANGE_SOURCES:$(SRCDIR)/%.c=$(OBJDIR)/%.o) $(GENERAL_SOURCES:$(SRCDIR)/%.c=$(OBJDIR)/%.o) $(LIB_SOURCES:$(LIBDIR)/%.c=$(OBJDIR)/%.o) $(OBJDIR)/$(CLIENT)/api.o
...
$(OBJDIR)/$(EXCHANGE)/$(EXCHANGE).o : $(APILIB)/$(SHARED_LIB_NAME_FULL) $(APILIB)/oidc-agent.h
$(OBJDIR)/%.o : $(SRCDIR)/%.c
...
$(BINDIR)/$(EXCHANGE): create_obj_dir_structure $(EXCHANGE_OBJECTS) $(APILIB)/$(SHARED_LIB_NAME_FULL) $(APILIB)/oidc-agent.h
...
$(BIN_PATH)/bin/$(EXCHANGE): $(BINDIR)/$(EXCHANGE)
	@install -D $< $@
...
$(MAN_PATH)/man1/$(CLIENT).1: $(MANDIR)/$(CLIENT).1
	@install -D $< $@
$(MAN_PATH)/man1/$(EXCHANGE).1: $(MANDIR)/$(EXCHANGE).1
	@install -D $< $@
